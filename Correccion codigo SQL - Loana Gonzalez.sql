...
Nombre_del_producto	Varchar (250) not null,	
Codigo_de_barras	Varchar (30) PRIMARY KEY not null,	
...
CREATE PROCEDURE Fecha_creacion_lista_compras(IN p_Fecha_de_creacion DATETIME)
BEGIN
    SELECT * 
    FROM Lista_de_compras
    WHERE Fecha_de_creacion > p_Fecha_de_creacion;
END$$
...
CREATE PROCEDURE ObtenerProductosAgotados(IN p_Disponibilidad VARCHAR(100))
BEGIN
    SELECT * 
    FROM Productos
    WHERE Disponibilidad = p_Disponibilidad;
END$$
